    ...
    <section className="page rules">
      ...
      <div className="card">
        ...
        <ul className="legend legend-block">
          ...
          <li>↑ / ↓ 箭头：数值型属性提示答案比你猜的更大或更小；位次列提示答案的位次比你猜的更靠前（↑）或更靠后（↓）</li>
        </ul>
      </div>
      ...
      <div className="card">
        ...
        <dl className="dims">
          ...
          <dt>位次</dt>
          <dd>
            全部位次落在同一条强弱阶梯上：主角 &gt; 一号位 &gt; 二号位 &gt; … &gt; 七号位 &gt; 次要 &gt; 配角。
            阶梯上相邻判为接近，并用 ↑ / ↓ 提示答案的位次更靠前还是更靠后
          </dd>
          <dt>发色 / 瞳色</dt>
          <dd>
      ...
      <div className="card">
        <h2>联机对战</h2>
        <p>
          <strong>联机对战需要先登录账号</strong>，房间里显示的名字直接取自账号用户名（想换名字到账号中心改用户名）。
      ...
      <div className="card">
        <h2>账号与战绩</h2>
        <p>
          单人模式不登录也能玩，只是不留记录；<strong>联机对战必须登录</strong>。
          <strong>登录后单人对局与联机对战会自动记录</strong>，在个人主页能看到胜率、平均猜测次数、连胜和猜中次数分布。
        </p>
        <dl className="dims">
          <dt>匿名局</dt>
          <dd>未登录时开的单人局不写入任何战绩</dd>
          <dt>每日一柚</dt>
          ...
          <dt>改用户名</dt>
          <dd>
            账号中心可随时改。用户名同时是登录名与联机显示名，改完要用新名字登录；
            战绩不受影响，历史对手名仍保留当时的写法
          </dd>
        </dl>
      </div>
