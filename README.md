...
- 🟩 **绿色** —— 该属性与答案完全一致
- 🟨 **黄色** —— 接近（位次在强弱阶梯上相邻、年份相差 ≤3 年、爆闪次数相差 ≤3 次、发色 / 瞳色属于同一色系，或同一位声优的其他化名）
- ⬛ **灰色** —— 不一致
- ↑↓ **箭头** —— 数值型属性提示答案比你猜的更大或更小；位次列提示答案的位次更靠前（↑）或更靠后（↓）

**位次**是一条完整的强弱阶梯，见 [`RANK_ORDER`](shared/src/gameService.ts)：

```
主角 > 一号位 > 二号位 > 三号位 > 四号位 > 五号位 > 六号位 > 七号位 > 次要 > 配角
```

阶梯上相邻的两个位次判 🟨 接近（如答案是一号位、猜到主角或二号位），跨度更大则判灰，两种情况都会给出 ↑ / ↓ 方向。`六号位` 目前无角色但仍占一个序号，免得五号位与七号位被误判为相邻。

发色与瞳色的**同色系**判定见 [`shared/src/colorGroups.ts`](shared/src/colorGroups.ts)：按色相分为红粉、紫、蓝、绿、暖黄（黄 / 金 / 橙 / 橘）、棕褐、暗色（黑 / 灰）、亮色（白 / 银 / 灰）几组。答案是橙发时猜到黄发即标黄；`青` 同属蓝与绿，`灰` 连接暗色与亮色，渐变 / 挑染色（如 `蓝粉渐变`、`棕橙挑染`）同时归入两个色相组；`黑` 与 `白` 不算同色系，瞳色 `未知` 永不判接近。

猜测表格最左侧一列显示猜过角色的立绘（与图鉴共用 `thumb` 档资源），缺立绘的角色退化为姓名首字占位。对手视角只保留占位符，不会泄露答案。
...
- [`shared/src/colorGroups.ts`](shared/src/colorGroups.ts) —— 发色 / 瞳色的同色系分组，用于「接近」判定
- [`client/public/portraits/`](client/public/portraits) —— 角色立绘，全 167 位角色各两档 webp（`card/` 高 400，`thumb/` 高 128，统一 3:4），按角色 id 命名

猜测时若两个角色的 CV 是同一位声优的不同化名，声优列判为 🟨 接近；占位值 `无`（男主角）与 `未知` 不参与同人判定。发色 / 瞳色同理，同色系判 🟨 接近。
...
图鉴支持卡片与表格两种视图，切换状态存 localStorage：卡片视图用 `card` 档立绘，表格视图用 `thumb` 档。猜测表格的立绘列同样取 `thumb` 档。
